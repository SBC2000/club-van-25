...
(function (window, document, $) {
	...
	Croppic.prototype = {
		...
		init: function () {
			...
			if (!$.isEmptyObject(that.options.loadCroppedPicture)){
				that.loadCroppedPicture(that.options.loadCroppedPicture);
			}
		...
		createImgUploadControls: function(){
			...
			if( $.isEmptyObject(that.croppedImg) || that.options.disableRemove){ cropControlRemoveCroppedImage=''; }
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
				...
				that.loadCroppedPicture(response.url);
		...
		loadCroppedPicture:function(url){
			var that = this;
			
			that.obj.append('<img class="croppedImg" src="' + url + '">');
			if (that.options.outputUrlId !== '') { $('#' + that.options.outputUrlId).val(url); }
